...
NNpos([stepdata[4],posactual[4]])
loss() = sum(abs, NNfit[i]- NNpos([stepdata[i],posactual[i]]) for i in 1:51)
loss()
...
cb = function () #callback function to observe training
  ...
  if iter % 1000 == 0
    display(loss())
    NNinput=hcat(stepdata,t2)
    NNoutput = map(i -> NNpos(NNinput[i,:]), 1:51)
    NNplot = posmodel .+ NNoutput
    plot!(t2, NNplot)
  end
end
